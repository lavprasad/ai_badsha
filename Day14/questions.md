...
Someone writes code for **Why every experiment needs a commit** and hits a bug in production.
...
Read the example for **Branching and merging** in `examples/` without running it.
...
For **Never commit data or secrets** the rule is:

> Log the data version alongside the code version — data changes silently, code changes loudly.
...
You are building a real system and **Seeding every random source** is on the table.
...
A colleague's code involving **Publishing a project to GitHub** produces results that look plausible but are wrong.
