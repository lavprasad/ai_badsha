...
Someone writes code for **Joint image-text embedding space** and hits a bug in production.
...
Read the example for **Zero-shot classification** in `examples/` without running it.
...
For **Image search by description** the rule is:

> Zero-shot quality depends heavily on prompt wording — 'a photo of a {}' beats a bare noun.
...
You are building a real system and **Visual question answering** is on the table.
...
A colleague's code involving **Building a semantic image search** produces results that look plausible but are wrong.
