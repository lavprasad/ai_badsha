...
Someone writes code for **Goal: grounded answers over your own documents** and hits a bug in production.
...
Read the example for **Embedding and index construction** in `examples/` without running it.
...
For **Grounded generation with citations** the rule is:

> Always show the source of each retrieved chunk in the answer so users can verify it.
...
You are building a real system and **Evaluation set and scoring harness** is on the table.
...
A colleague's code involving **Deploying it behind an API** produces results that look plausible but are wrong.
