...
Someone writes code for **Mean, median and mode** and hits a bug in production.
...
Read the example for **Quantiles, percentiles and IQR** in `examples/` without running it.
...
For **Covariance and correlation** the rule is:

> Quote median + IQR for skewed data, mean + std only for roughly symmetric data.
...
You are building a real system and **Robust statistics and outliers** is on the table.
...
A colleague's code involving **Summarising a dataset honestly** produces results that look plausible but are wrong.
