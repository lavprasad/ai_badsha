...
Someone writes code for **Variables, types and dynamic typing** and hits a bug in production.
...
Read the example for **Lists, tuples, sets, dicts** in `examples/` without running it.
...
For **if/elif/else and truthiness** the rule is:

> `if x:` and `if x is not None:` differ for 0, '' and empty containers. Pick deliberately.
...
You are building a real system and **Functions, arguments and defaults** is on the table.
...
A colleague's code involving **Reading the standard library docs** produces results that look plausible but are wrong.
