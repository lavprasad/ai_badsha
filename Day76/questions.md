...
Someone writes code for **Inputs, weights, bias** and hits a bug in production.
...
Read the example for **Activation functions** in `examples/` without running it.
...
For **XOR and why one layer is not enough** the rule is:

> Depth without non-linearity is width. Check that every hidden layer has an activation.
...
You are building a real system and **Matrix form of a layer** is on the table.
...
A colleague's code involving **A forward pass in pure NumPy** produces results that look plausible but are wrong.
