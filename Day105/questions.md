...
Someone writes code for **Pixels, channels and bit depth** and hits a bug in production.
...
Read the example for **Image file formats and compression** in `examples/` without running it.
...
For **Resizing and interpolation methods** the rule is:

> Check the channel order (RGB vs BGR) and the value range (0-255 vs 0-1) before every model call.
...
You are building a real system and **Normalisation for pretrained models** is on the table.
...
A colleague's code involving **Building an image loading utility** produces results that look plausible but are wrong.
