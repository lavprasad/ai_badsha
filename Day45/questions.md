...
Someone writes code for **The scikit-learn estimator API** and hits a bug in production.
...
Read the example for **Loading a built-in dataset** in `examples/` without running it.
...
For **Training a logistic regression** the rule is:

> Clip the sigmoid input — `exp` of a large negative number overflows and returns NaN.
...
You are building a real system and **Comparing against a dummy baseline** is on the table.
...
A colleague's code involving **The seven-line template you will reuse forever** produces results that look plausible but are wrong.
