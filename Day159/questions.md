...
Someone writes code for **Explicit planning steps** and hits a bug in production.
...
Read the example for **Subagents and delegation** in `examples/` without running it.
...
For **Context management across long runs** the rule is:
...
You are building a real system and **Detecting loops and stalls** is on the table.
...
A colleague's code involving **Evaluating agent trajectories** produces results that look plausible but are wrong.
