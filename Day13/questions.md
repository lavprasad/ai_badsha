...
Someone writes code for **Why SQL is still where the data lives** and hits a bug in production.
...
Read the example for **GROUP BY and HAVING** in `examples/` without running it.
...
For **Window functions for lag features** the rule is:

> Every feature must use `.shift(1)` or later — no row may see its own future.
...
You are building a real system and **sqlite3 from Python** is on the table.
...
A colleague's code involving **Query performance basics** produces results that look plausible but are wrong.
