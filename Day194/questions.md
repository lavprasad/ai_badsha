...
Someone writes code for **Choosing a paper with released code or clear method** and hits a bug in production.
...
Read the example for **Restating the core idea in one paragraph** in `examples/` without running it.
...
For **Reproducing the headline number** the rule is:
...
You are building a real system and **Ablating the key component** is on the table.
...
A colleague's code involving **Publishing the repository** produces results that look plausible but are wrong.
