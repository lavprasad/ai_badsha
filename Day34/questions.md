...
Someone writes code for **Goal: a tiny numeric library you understand fully** and hits a bug in production.
...
Read the example for **Numeric gradient utility** in `examples/` without running it.
...
For **Linear regression via normal equations** the rule is:

> Cosine similarity ignores magnitude; Euclidean distance does not. Pick the one that matches your question.
...
You are building a real system and **Logistic regression from scratch** is on the table.
...
A colleague's code involving **What you now never have to take on faith** produces results that look plausible but are wrong.
