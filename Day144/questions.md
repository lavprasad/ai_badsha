...
Someone writes code for **Parametric vs retrieved knowledge** and hits a bug in production.
...
Read the example for **Model editing** in `examples/` without running it.
...
For **Entity linking** the rule is:

> If a fact can change tomorrow, retrieve it. If it cannot, the weights may hold it.
...
You are building a real system and **Temporal reasoning failures** is on the table.
...
A colleague's code involving **Designing the knowledge boundary** produces results that look plausible but are wrong.
