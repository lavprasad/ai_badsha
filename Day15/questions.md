...
Someone writes code for **Framing the tool: what question does it answer** and hits a bug in production.
...
Read the example for **Auto-detecting column types** in `examples/` without running it.
...
For **Summary statistics per column type** the rule is:

> Spend an hour looking at wrong predictions before you spend a day tuning hyperparameters.
...
You are building a real system and **Generating plots to a folder** is on the table.
...
A colleague's code involving **Packaging and reusing the tool** produces results that look plausible but are wrong.
