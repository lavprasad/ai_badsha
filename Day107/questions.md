...
Someone writes code for **Detection vs classification** and hits a bug in production.
...
Read the example for **Intersection over union** in `examples/` without running it.
...
For **Two-stage detectors: R-CNN family** the rule is:
...
You are building a real system and **Non-maximum suppression** is on the table.
...
A colleague's code involving **Fine-tuning a detector** produces results that look plausible but are wrong.
