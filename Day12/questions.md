...
Someone writes code for **seaborn on top of matplotlib** and hits a bug in production.
...
Read the example for **Pairplots and correlation heatmaps** in `examples/` without running it.
...
For **Scatter with hue and size encoding** the rule is:

> Label the axes. An unlabelled plot is a decoration, not evidence.
...
You are building a real system and **Spotting skew and outliers visually** is on the table.
...
A colleague's code involving **Building a one-page EDA report** produces results that look plausible but are wrong.
