...
Someone writes code for **Wisdom of uncorrelated errors** and hits a bug in production.
...
Read the example for **Bagging** in `examples/` without running it.
...
For **Out-of-bag error estimation** the rule is:

> More trees never overfit — they only cost time. Depth and leaf size are the knobs that control fit.
...
You are building a real system and **Feature importance and its bias** is on the table.
...
A colleague's code involving **Random forest as the tabular default** produces results that look plausible but are wrong.
