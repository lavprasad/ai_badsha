...
Someone writes code for **When a straight line is the right model** and hits a bug in production.
...
Read the example for **Interpreting coefficients and units** in `examples/` without running it.
...
For **Heteroscedasticity** the rule is:

> Plot residuals against predictions — any visible pattern means the linear form is wrong.
...
You are building a real system and **Multicollinearity in real data** is on the table.
...
A colleague's code involving **House-price regression walkthrough** produces results that look plausible but are wrong.
