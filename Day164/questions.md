...
Someone writes code for **Defining success for a fuzzy task** and hits a bug in production.
...
Read the example for **Deterministic checks first** in `examples/` without running it.
...
For **Pairwise preference evaluation** the rule is:

> Alignment optimises a proxy for what humans want; the proxy can always be gamed.
...
You are building a real system and **Regression suites in CI** is on the table.
...
A colleague's code involving **Closing the loop from eval to fix** produces results that look plausible but are wrong.
