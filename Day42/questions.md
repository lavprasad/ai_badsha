...
Someone writes code for **What leakage actually is** and hits a bug in production.
...
Read the example for **Target leakage from future columns** in `examples/` without running it.
...
For **Group leakage: same entity both sides** the rule is:

> 0.999 AUC on a hard business problem is a bug report, not a result.
...
You are building a real system and **Leakage through feature selection** is on the table.
...
A colleague's code involving **Fixing a leak without starting over** produces results that look plausible but are wrong.
