...
Someone writes code for **Choosing a task LLMs genuinely help with** and hits a bug in production.
...
Read the example for **Retrieval or tools as needed** in `examples/` without running it.
...
For **Golden eval set from real inputs** the rule is:
...
You are building a real system and **Prompt injection hardening** is on the table.
...
A colleague's code involving **Demo video and documentation** produces results that look plausible but are wrong.
