...
**The 2017 idea: attention is all you need** — the classic failure is:
...
The example for **Self-attention block** is `examples/03_self_attention_block.py`.
...
Key idea to check yourself against: The 1/sqrt(d) scale is not cosmetic — without it softmax saturates and gradients die.
...
**Encoder stack** in one line: A transformer block is attention + feed-forward, each wrapped in a residual connection and a LayerNorm.
...
For **Drawing the block from memory**, check in this order:
