...
Someone writes code for **Images as arrays** and hits a bug in production.
...
Read the example for **Resizing, cropping, normalising** in `examples/` without running it.
...
For **Edge detection and filters** the rule is:
...
You are building a real system and **Classical image classification** is on the table.
...
A colleague's code involving **Loading an image dataset efficiently** produces results that look plausible but are wrong.
