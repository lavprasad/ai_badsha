...
Someone writes code for **Encoder, bottleneck, decoder** and hits a bug in production.
...
Read the example for **Undercomplete vs overcomplete** in `examples/` without running it.
...
For **Sparse autoencoders** the rule is:

> Reconstruction error is a ready-made anomaly score — no labels required.
...
You are building a real system and **Variational autoencoders** is on the table.
...
A colleague's code involving **Building an anomaly detector with one** produces results that look plausible but are wrong.
