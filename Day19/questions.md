...
Someone writes code for **The derivative as a rate of change** and hits a bug in production.
...
Read the example for **Partial derivatives** in `examples/` without running it.
...
For **Directional derivatives** the rule is:
...
You are building a real system and **The Jacobian** is on the table.
...
A colleague's code involving **Hand-deriving a two-layer gradient** produces results that look plausible but are wrong.
