...
Someone writes code for **Batch vs streaming ingestion** and hits a bug in production.
...
Read the example for **Orchestration with Airflow or Prefect** in `examples/` without running it.
...
For **Partitioning and file formats** the rule is:

> Idempotent + partitioned = safe backfills. Append-only pipelines make every rerun a data corruption event.
...
You are building a real system and **Incremental processing** is on the table.
...
A colleague's code involving **Cost of storage and compute** produces results that look plausible but are wrong.
