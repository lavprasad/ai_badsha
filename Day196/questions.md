...
Someone writes code for **Where papers appear: arXiv and conferences** and hits a bug in production.
...
Read the example for **Distinguishing contribution from packaging** in `examples/` without running it.
...
For **Checking baselines for fairness** the rule is:

> Dropping the sensitive attribute does not remove bias; proxies (pincode, name) carry it right back in.
...
You are building a real system and **Following citation trails** is on the table.
...
A colleague's code involving **Staying current without drowning** produces results that look plausible but are wrong.
