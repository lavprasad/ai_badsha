...
Someone writes code for **Video as a sequence of frames** and hits a bug in production.
...
Read the example for **Optical flow** in `examples/` without running it.
...
For **3D convolutions** the rule is:
...
You are building a real system and **Object tracking** is on the table.
...
A colleague's code involving **A simple activity classifier** produces results that look plausible but are wrong.
