...
Someone writes code for **Autoregressive generation** and hits a bug in production.
...
Read the example for **Temperature** in `examples/` without running it.
...
For **Top-p nucleus sampling** the rule is:
...
You are building a real system and **Beam search for constrained tasks** is on the table.
...
A colleague's code involving **Choosing decoding settings per task** produces results that look plausible but are wrong.
