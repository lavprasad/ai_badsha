...
Someone writes code for **Goal: a model running in production with monitoring** and hits a bug in production.
...
Read the example for **FastAPI service with validation** in `examples/` without running it.
...
For **CI with tests and an eval gate** the rule is:

> 50 real examples you curated beat 5000 synthetic ones nobody checked.
...
You are building a real system and **Structured request/response logging** is on the table.
...
A colleague's code involving **Load test and cost report** produces results that look plausible but are wrong.
