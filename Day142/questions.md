...
Someone writes code for **Why standard benchmarks mislead** and hits a bug in production.
...
Read the example for **Task-specific automatic metrics** in `examples/` without running it.
...
You are building a real system and **Pairwise comparison and Elo** is on the table.
...
A colleague's code involving **An eval harness you run on every change** produces results that look plausible but are wrong.
