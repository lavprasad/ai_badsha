...
Someone writes code for **The conditional independence assumption** and hits a bug in production.
...
Read the example for **Multinomial Naive Bayes for text** in `examples/` without running it.
...
For **Laplace smoothing** the rule is:
...
You are building a real system and **Speed as a feature** is on the table.
...
A colleague's code involving **When to prefer it over logistic regression** produces results that look plausible but are wrong.
