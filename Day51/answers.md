...
**The conditional independence assumption** — the classic failure is:
...
The example for **Multinomial Naive Bayes for text** is `examples/03_multinomial_naive_bayes_for_text.py`.
...
**Speed as a feature** in one line: Bayes' rule updates a belief with evidence: posterior = likelihood x prior / evidence.
...
For **When to prefer it over logistic regression**, check in this order:
