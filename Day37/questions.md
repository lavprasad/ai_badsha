...
Someone writes code for **First five questions of any dataset** and hits a bug in production.
...
Read the example for **Univariate distributions** in `examples/` without running it.
...
For **Correlation matrices and their limits** the rule is:

> Initialise B to zeros so the adapted model starts exactly equal to the base model.
...
You are building a real system and **Time trends in the data** is on the table.
...
A colleague's code involving **Turning EDA into modelling hypotheses** produces results that look plausible but are wrong.
