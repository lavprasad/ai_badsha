...
Someone writes code for **Why free text is hard to use** and hits a bug in production.
...
Read the example for **Tool/function calling** in `examples/` without running it.
...
For **Pydantic models for responses** the rule is:
...
You are building a real system and **Handling partial and malformed output** is on the table.
...
A colleague's code involving **A robust extraction service** produces results that look plausible but are wrong.
