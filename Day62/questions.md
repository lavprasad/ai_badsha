...
Someone writes code for **Global vs local explanation** and hits a bug in production.
...
Read the example for **Impurity importance and its bias** in `examples/` without running it.
...
For **Partial dependence plots** the rule is:
...
You are building a real system and **SHAP values** is on the table.
...
A colleague's code involving **Explanation is not causation** produces results that look plausible but are wrong.
