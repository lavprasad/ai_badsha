...
Someone writes code for **Frequentist vs Bayesian views** and hits a bug in production.
...
Read the example for **Conjugate priors: Beta-Binomial** in `examples/` without running it.
...
For **Credible intervals vs confidence intervals** the rule is:

> Rare events make precision collapse no matter how good the classifier looks on accuracy.
...
You are building a real system and **Naive Bayes as a Bayesian classifier** is on the table.
...
A colleague's code involving **A Beta-Binomial update in code** produces results that look plausible but are wrong.
