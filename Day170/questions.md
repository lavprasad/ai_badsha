...
Someone writes code for **Code as a modality** and hits a bug in production.
...
Read the example for **Repository-level context** in `examples/` without running it.
...
For **Code review assistance** the rule is:

> Feed the test failure text straight back as context — it is the highest-signal prompt you have.
...
You are building a real system and **Execution feedback loops** is on the table.
...
A colleague's code involving **Building a code assistant workflow** produces results that look plausible but are wrong.
