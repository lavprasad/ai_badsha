...
Someone writes code for **Loss as the objective you actually optimise** and hits a bug in production.
...
Read the example for **Mean absolute error and Huber** in `examples/` without running it.
...
For **Categorical cross-entropy** the rule is:
...
You are building a real system and **Class weights inside the loss** is on the table.
...
A colleague's code involving **Custom losses for business costs** produces results that look plausible but are wrong.
