...
Someone writes code for **Overfit a single batch first** and hits a bug in production.
...
Read the example for **Verify the data reaching the model** in `examples/` without running it.
...
For **NaN loss: causes and fixes** the rule is:

> Compute the fill statistic on the TRAIN split only, then apply it to test.
...
You are building a real system and **Learning rate diagnosis from the curve** is on the table.
...
A colleague's code involving **A deep learning debugging checklist** produces results that look plausible but are wrong.
