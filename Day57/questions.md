...
Someone writes code for **Parameters vs hyperparameters** and hits a bug in production.
...
Read the example for **Grid search** in `examples/` without running it.
...
For **Bayesian optimisation** the rule is:
...
You are building a real system and **Successive halving** is on the table.
...
A colleague's code involving **Logging every trial** produces results that look plausible but are wrong.
