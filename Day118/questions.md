...
Someone writes code for **Requirements: accuracy, latency, cost** and hits a bug in production.
...
Read the example for **Annotation strategy and quality control** in `examples/` without running it.
...
For **Handling class imbalance in vision** the rule is:

> State one assumption `Handling class imbalance in vision` makes about your data before you use it.
...
You are building a real system and **Human-in-the-loop review** is on the table.
...
A colleague's code involving **Writing the system design document** produces results that look plausible but are wrong.
