...
Someone writes code for **The idea: similarity is prediction** and hits a bug in production.
...
Read the example for **Distance metrics and weighting** in `examples/` without running it.
...
For **kNN for regression** the rule is:

> Scale your features first — kNN is pure distance, so units decide the answer.
...
You are building a real system and **KD-trees and ball trees** is on the table.
...
A colleague's code involving **Implementing kNN from scratch** produces results that look plausible but are wrong.
