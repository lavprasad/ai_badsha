...
Someone writes code for **Why models need tokens, not words** and hits a bug in production.
...
Read the example for **Character-level trade-offs** in `examples/` without running it.
...
For **WordPiece and SentencePiece** the rule is:

> Roughly 1 token ~ 4 characters of English; other languages cost far more tokens per word.
...
You are building a real system and **Special tokens: BOS, EOS, PAD, UNK** is on the table.
...
A colleague's code involving **Implementing BPE merges by hand** produces results that look plausible but are wrong.
