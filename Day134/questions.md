...
Someone writes code for **Prefill vs decode phases** and hits a bug in production.
...
Read the example for **Continuous batching** in `examples/` without running it.
...
For **Quantised inference: GPTQ, AWQ, GGUF** the rule is:

> Batch requests where latency allows — GPU throughput collapses on batch size 1.
...
You are building a real system and **Throughput vs latency trade-offs** is on the table.
...
A colleague's code involving **Choosing hosted vs self-hosted** produces results that look plausible but are wrong.
