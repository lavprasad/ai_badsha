...
Someone writes code for **Goal: a small model specialised to one task** and hits a bug in production.
...
Read the example for **Formatting an SFT dataset** in `examples/` without running it.
...
For **Training run and monitoring** the rule is:
...
You are building a real system and **Comparing against a prompted large model** is on the table.
...
A colleague's code involving **Packaging the adapter** produces results that look plausible but are wrong.
