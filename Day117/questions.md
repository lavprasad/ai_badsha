...
Someone writes code for **Depth from stereo** and hits a bug in production.
...
Read the example for **Point clouds** in `examples/` without running it.
...
For **PointNet** the rule is:

> Stereo depth error grows with the square of distance — far objects are barely measurable.
...
You are building a real system and **Gaussian splatting** is on the table.
...
A colleague's code involving **Evaluating 3D reconstructions** produces results that look plausible but are wrong.
