...
Someone writes code for **The linear model equation** and hits a bug in production.
...
Read the example for **Deriving the normal equations** in `examples/` without running it.
...
For **Adding a bias term correctly** the rule is:
...
You are building a real system and **Multicollinearity and its symptoms** is on the table.
...
A colleague's code involving **Interpreting coefficients honestly** produces results that look plausible but are wrong.
