...
**LeNet and AlexNet** — the classic failure is:
...
The example for **Inception and multi-scale** is `examples/03_inception_and_multi_scale.py`.
...
**EfficientNet and compound scaling** in one line: A convolution slides a small learned filter over the image, so the same edge detector works anywhere in the frame.

Use it when its assumptions hold and the cost is justified. Reach for something
simpler when they do not — a baseline you understand beats a sophisticated method
you cannot debug at 2am. The right answer names *the assumption*, not the tool.

Watch out for: Forgetting the channel dimension and feeding (H,W) where the layer expects (N,C,H,W).

---

### A5. Debug it

For **Reading an architecture diagram**, check in this order:
