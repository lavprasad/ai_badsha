...
Someone writes code for **Why notebooks do not ship** and hits a bug in production.
...
Read the example for **Configuration files over hard-coded values** in `examples/` without running it.
...
For **Logging instead of print** the rule is:

> 'Restart kernel and run all' is the only honest test that a notebook works.
...
You are building a real system and **Packaging with pyproject.toml** is on the table.
...
A colleague's code involving **Refactoring a notebook into modules** produces results that look plausible but are wrong.
