...
Someone writes code for **Why RAG instead of fine-tuning** and hits a bug in production.
...
Read the example for **Document loading and parsing** in `examples/` without running it.
...
For **Chunk size and overlap trade-offs** the rule is:
...
You are building a real system and **Storing vectors with metadata** is on the table.
...
A colleague's code involving **A minimal RAG in 50 lines** produces results that look plausible but are wrong.
