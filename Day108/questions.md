...
Someone writes code for **Semantic vs instance vs panoptic** and hits a bug in production.
...
Read the example for **U-Net architecture** in `examples/` without running it.
...
For **Dice and IoU losses** the rule is:

> IoU >= 0.5 is the usual 'correct detection' threshold; report mAP, not accuracy.
...
You are building a real system and **Segment Anything and promptable segmentation** is on the table.
...
A colleague's code involving **Evaluating segmentation quality** produces results that look plausible but are wrong.
