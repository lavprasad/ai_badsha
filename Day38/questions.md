...
Someone writes code for **Missing values: why before how** and hits a bug in production.
...
Read the example for **Simple imputation: mean, median, mode** in `examples/` without running it.
...
For **Missingness indicator columns** the rule is:
...
You are building a real system and **Winsorising vs removing** is on the table.
...
A colleague's code involving **A reusable cleaning function** produces results that look plausible but are wrong.
