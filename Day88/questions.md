...
Someone writes code for **Why dense layers fail on images** and hits a bug in production.
...
Read the example for **Kernels, stride and padding** in `examples/` without running it.
...
For **Receptive field** the rule is:

> Output size = (in - kernel + 2*pad)/stride + 1. Print shapes when a layer refuses to connect.
...
You are building a real system and **Parameter sharing and translation equivariance** is on the table.
...
A colleague's code involving **Implementing a conv by hand** produces results that look plausible but are wrong.
