...
Someone writes code for **Parsing dates with to_datetime** and hits a bug in production.
...
Read the example for **Rolling windows and expanding** in `examples/` without running it.
...
For **String methods with .str** the rule is:
...
You are building a real system and **Categorical dtype for memory** is on the table.
...
A colleague's code involving **A monthly report in 20 lines** produces results that look plausible but are wrong.
