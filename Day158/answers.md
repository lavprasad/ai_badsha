...
**Designing a good tool interface** — the classic failure is:
...
The example for **Search and retrieval tools** is `examples/03_search_and_retrieval_tools.py`.
...
**Database query tools with allowlists** in one line: An agent is a loop: the model picks a tool, your code runs it, the result goes back into context, repeat until done.
...
For **Testing tools independently**, check in this order:
