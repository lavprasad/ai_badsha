...
Someone writes code for **Designing a good tool interface** and hits a bug in production.
...
Read the example for **Search and retrieval tools** in `examples/` without running it.
...
For **File system access controls** the rule is:

> Always bound the loop. An unbounded agent burns money and finds creative ways to fail.
...
You are building a real system and **Database query tools with allowlists** is on the table.
...
A colleague's code involving **Testing tools independently** produces results that look plausible but are wrong.
