...
Someone writes code for **Content-based filtering** and hits a bug in production.
...
Read the example for **User-item matrices and sparsity** in `examples/` without running it.
...
For **Implicit vs explicit feedback** the rule is:

> Evaluate recommenders with ranking metrics (precision@k, NDCG), not RMSE on ratings.
...
You are building a real system and **Popularity bias and feedback loops** is on the table.
...
A colleague's code involving **A movie recommender walkthrough** produces results that look plausible but are wrong.
