...
Someone writes code for **What makes something an agent** and hits a bug in production.
...
Read the example for **Tool definitions and schemas** in `examples/` without running it.
...
For **Iteration limits** the rule is:
...
You are building a real system and **State between steps** is on the table.
...
A colleague's code involving **Building a two-tool agent from scratch** produces results that look plausible but are wrong.
