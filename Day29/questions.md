...
Someone writes code for **Floating point representation** and hits a bug in production.
...
Read the example for **Overflow and underflow** in `examples/` without running it.
...
For **Epsilon in denominators** the rule is:

> Compare floats with a tolerance (`np.isclose`), never with `==`.
...
You are building a real system and **Deterministic seeds vs true randomness** is on the table.
...
A colleague's code involving **Debugging a silently wrong computation** produces results that look plausible but are wrong.
