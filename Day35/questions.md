...
Someone writes code for **Starting from the decision, not the data** and hits a bug in production.
...
Read the example for **Classification vs regression vs ranking** in `examples/` without running it.
...
For **Choosing an evaluation metric** the rule is:
...
You are building a real system and **Baselines you must beat** is on the table.
...
A colleague's code involving **Writing a one-page problem statement** produces results that look plausible but are wrong.
