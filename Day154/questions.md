...
Someone writes code for **Structure-aware chunking** and hits a bug in production.
...
Read the example for **PDF parsing pitfalls** in `examples/` without running it.
...
For **Code and config files** the rule is:
...
You are building a real system and **Parent-document retrieval** is on the table.
...
A colleague's code involving **Choosing chunk size empirically** produces results that look plausible but are wrong.
