...
Someone writes code for **Sequential error correction** and hits a bug in production.
...
Read the example for **Gradient boosting as gradient descent in function space** in `examples/` without running it.
...
For **Tree depth in boosting** the rule is:

> Low learning rate + many trees + early stopping beats high learning rate + few trees.
...
You are building a real system and **XGBoost, LightGBM, CatBoost compared** is on the table.
...
A colleague's code involving **Why boosting still beats deep nets on tables** produces results that look plausible but are wrong.
