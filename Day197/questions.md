...
Someone writes code for **Research scientist vs ML engineer vs data scientist** and hits a bug in production.
...
Read the example for **Data engineer and platform roles** in `examples/` without running it.
...
For **Skills that transfer between them** the rule is:

> One project you can defend end to end beats ten tutorials on your CV.
...
You are building a real system and **Startup vs enterprise trade-offs** is on the table.
...
A colleague's code involving **Choosing your next two years** produces results that look plausible but are wrong.
