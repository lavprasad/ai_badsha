...
Someone writes code for **The trust boundary in an LLM app** and hits a bug in production.
...
Read the example for **Indirect injection via documents and web pages** in `examples/` without running it.
...
For **Confused deputy problems** the rule is:

> Delimit untrusted content and enforce authorisation in code — a prompt is not a security boundary.
...
You are building a real system and **Authorisation in code, not prompts** is on the table.
...
A colleague's code involving **A threat model for your app** produces results that look plausible but are wrong.
