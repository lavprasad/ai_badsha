...
Someone writes code for **Encoder-only: BERT** and hits a bug in production.
...
Read the example for **Decoder-only: GPT family** in `examples/` without running it.
...
For **Encoder-decoder: T5 and BART** the rule is:
...
You are building a real system and **Which family for which task** is on the table.
...
A colleague's code involving **Reading a model card** produces results that look plausible but are wrong.
