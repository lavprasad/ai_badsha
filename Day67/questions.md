...
Someone writes code for **Time-to-event framing** and hits a bug in production.
...
Read the example for **Kaplan-Meier curves** in `examples/` without running it.
...
For **Cox proportional hazards** the rule is:

> A censored row still carries information: it survived at least that long. Never drop it.
...
You are building a real system and **Features that change over time** is on the table.
...
A colleague's code involving **When plain classification is enough** produces results that look plausible but are wrong.
