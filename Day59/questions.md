...
Someone writes code for **What clustering can and cannot tell you** and hits a bug in production.
...
Read the example for **Choosing k: elbow and silhouette** in `examples/` without running it.
...
For **Limitations: shape and scale assumptions** the rule is:

> Silhouette near 1 means tight, well-separated clusters; near 0 means the boundaries are arbitrary.
...
You are building a real system and **Hierarchical clustering and dendrograms** is on the table.
...
A colleague's code involving **Customer segmentation walkthrough** produces results that look plausible but are wrong.
