...
Someone writes code for **Text preprocessing pipeline** and hits a bug in production.
...
Read the example for **Stopwords, stemming, lemmatisation** in `examples/` without running it.
...
For **TF-IDF** the rule is:

> TF-IDF + logistic regression is the baseline every LLM text classifier must beat to be worth its cost.
...
You are building a real system and **Character n-grams for noisy text** is on the table.
...
A colleague's code involving **The baseline every LLM must beat** produces results that look plausible but are wrong.
