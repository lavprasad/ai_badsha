...
Someone writes code for **Unit tests for transforms** and hits a bug in production.
...
Read the example for **Model contract tests: shape and range** in `examples/` without running it.
...
For **Metamorphic testing** the rule is:

> Every test that touches randomness must pass an explicit seed. Global seeding is not enough under parallelism.
...
You are building a real system and **Testing training runs cheaply** is on the table.
...
A colleague's code involving **A test suite you actually run** produces results that look plausible but are wrong.
