...
Someone writes code for **Text-to-image workflow** and hits a bug in production.
...
Read the example for **Negative prompts** in `examples/` without running it.
...
For **Guidance scale and steps** the rule is:

> More sampling steps means better quality and linearly more compute — that is the whole trade.
...
You are building a real system and **LoRA for style adaptation** is on the table.
...
A colleague's code involving **Copyright and dataset questions** produces results that look plausible but are wrong.
