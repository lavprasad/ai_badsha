...
Someone writes code for **Eigenvectors as invariant directions** and hits a bug in production.
...
Read the example for **The characteristic equation** in `examples/` without running it.
...
For **Singular value decomposition** the rule is:

> Singular values sorted descending tell you how many dimensions actually carry information.
...
You are building a real system and **SVD for image compression** is on the table.
...
A colleague's code involving **Connection to LoRA adapters** produces results that look plausible but are wrong.
