...
Someone writes code for **What to record for every run** and hits a bug in production.
...
Read the example for **Artefact storage** in `examples/` without running it.
...
For **Seeding everything** the rule is:
...
You are building a real system and **Comparing runs** is on the table.
...
A colleague's code involving **A tracking habit that costs 5 minutes** produces results that look plausible but are wrong.
