...
Someone writes code for **Extractive vs abstractive summarisation** and hits a bug in production.
...
Read the example for **Map-reduce summarisation** in `examples/` without running it.
...
For **Structured extraction with schemas** the rule is:

> Extract facts structurally and summarise prose separately — summarisation loses numbers first.
...
You are building a real system and **Evaluating summaries** is on the table.
...
A colleague's code involving **A meeting-notes summariser** produces results that look plausible but are wrong.
