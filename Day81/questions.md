...
Someone writes code for **Why a constant learning rate is rarely best** and hits a bug in production.
...
Read the example for **Exponential decay** in `examples/` without running it.
...
For **Warmup and why transformers need it** the rule is:
...
You are building a real system and **Reduce on plateau** is on the table.
...
A colleague's code involving **Reading a loss curve to pick a schedule** produces results that look plausible but are wrong.
