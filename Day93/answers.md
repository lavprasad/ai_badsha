...
**The bottleneck problem in seq2seq** — the classic failure is:
...
The example for **Query, key and value** is `examples/03_query_key_and_value.py`.
...
**Self-attention vs cross-attention** in one line: Attention lets every token look at every other token and decide what matters.
...
For **Implementing attention in NumPy**, check in this order:
