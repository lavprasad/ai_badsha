...
**Embedding models today** — the classic failure is:

> Mixing vectors from two different embedding models in one index; the spaces are unrelated.
...
The example for **Normalisation and cosine similarity** is `examples/03_normalisation_and_cosine_similarity.py`.
...
**Metadata filtering** in one line: An embedding maps text to a dense vector where nearby means similar in meaning.
...
For **Building a semantic search over your notes**, check in this order:
