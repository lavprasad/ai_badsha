...
Someone writes code for **Decomposition into sub-prompts** and hits a bug in production.
...
Read the example for **Self-critique and revision loops** in `examples/` without running it.
...
For **Persona and expertise framing** the rule is:

> Put the output format last and show it as an example — models copy the nearest pattern.
...
You are building a real system and **Refusal and safe-completion design** is on the table.
...
A colleague's code involving **A prompt library with tests** produces results that look plausible but are wrong.
