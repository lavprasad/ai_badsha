...
Someone writes code for **Text plus image inputs** and hits a bug in production.
...
Read the example for **Document and chart understanding** in `examples/` without running it.
...
For **Audio-native models** the rule is:
...
You are building a real system and **Failure modes on fine detail** is on the table.
...
A colleague's code involving **A screenshot-to-data pipeline** produces results that look plausible but are wrong.
