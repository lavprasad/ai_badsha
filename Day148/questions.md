...
Someone writes code for **Anatomy of a chat completion request** and hits a bug in production.
...
Read the example for **max_tokens and stop conditions** in `examples/` without running it.
...
For **Retries, timeouts and backoff** the rule is:

> Jitter matters: without it, every client retries at the same instant and re-creates the outage.
...
You are building a real system and **Cost tracking per request** is on the table.
...
A colleague's code involving **A resilient API client wrapper** produces results that look plausible but are wrong.
