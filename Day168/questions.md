...
Someone writes code for **Capturing user feedback** and hits a bug in production.
...
Read the example for **Building a labelling loop** in `examples/` without running it.
...
For **Turning corrections into eval cases** the rule is:
...
You are building a real system and **Avoiding feedback loop bias** is on the table.
...
A colleague's code involving **Designing the loop from day one** produces results that look plausible but are wrong.
