...
Someone writes code for **What a vector database actually does** and hits a bug in production.
...
Read the example for **HNSW and IVF indexes** in `examples/` without running it.
...
For **Metadata filtering strategies** the rule is:
...
You are building a real system and **Options: FAISS, Chroma, Qdrant, pgvector** is on the table.
...
A colleague's code involving **Choosing one for your scale** produces results that look plausible but are wrong.
