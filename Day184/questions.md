...
Someone writes code for **When single-GPU stops working** and hits a bug in production.
...
Read the example for **Distributed data parallel** in `examples/` without running it.
...
For **Model parallelism and sharding** the rule is:
...
You are building a real system and **Mixed precision at scale** is on the table.
...
A colleague's code involving **Deciding whether you need any of this** produces results that look plausible but are wrong.
