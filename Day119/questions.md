...
Someone writes code for **Goal: a working vision service** and hits a bug in production.
...
Read the example for **Collecting and annotating data** in `examples/` without running it.
...
For **Training with augmentation** the rule is:
...
You are building a real system and **Inference API around the model** is on the table.
...
A colleague's code involving **Demo and documentation** produces results that look plausible but are wrong.
