...
Someone writes code for **Perceived vs actual latency** and hits a bug in production.
...
Read the example for **Parallelising independent calls** in `examples/` without running it.
...
For **Reducing prompt size** the rule is:
...
You are building a real system and **Caching retrieval results** is on the table.
...
A colleague's code involving **Meeting a latency SLA** produces results that look plausible but are wrong.
