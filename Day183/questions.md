...
Someone writes code for **What a model incident looks like** and hits a bug in production.
...
Read the example for **Immediate mitigation: rollback or disable** in `examples/` without running it.
...
For **Data pipeline failures** the rule is:
...
You are building a real system and **Communication with stakeholders** is on the table.
...
A colleague's code involving **Reducing mean time to recovery** produces results that look plausible but are wrong.
