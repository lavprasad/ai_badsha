...
Someone writes code for **Comparing models fairly** and hits a bug in production.
...
Read the example for **Repeated and stratified CV** in `examples/` without running it.
...
For **Statistical comparison of two models** the rule is:

> State one assumption `Statistical comparison of two models` makes about your data before you use it.
...
You are building a real system and **Nested CV for honest estimates** is on the table.
...
A colleague's code involving **Choosing simplest-that-works** produces results that look plausible but are wrong.
