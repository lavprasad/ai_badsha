...
Someone writes code for **Sample spaces and events** and hits a bug in production.
...
Read the example for **Independence and conditional probability** in `examples/` without running it.
...
For **Prior, likelihood, posterior** the rule is:
...
You are building a real system and **Random variables** is on the table.
...
A colleague's code involving **Monte Carlo estimation** produces results that look plausible but are wrong.
