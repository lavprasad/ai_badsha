...
Someone writes code for **RNNs for text** and hits a bug in production.
...
Read the example for **Bidirectional context** in `examples/` without running it.
...
For **Teacher forcing** the rule is:
...
You are building a real system and **The context bottleneck** is on the table.
...
A colleague's code involving **What still carries over** produces results that look plausible but are wrong.
