...
Someone writes code for **Starting from a known-good architecture** and hits a bug in production.
...
Read the example for **Skip connections by default** in `examples/` without running it.
...
For **Regularisation budget** the rule is:

> Scale features before regularising, or the penalty punishes whichever column happens to use small units.
...
You are building a real system and **Loss matching the output layer** is on the table.
...
A colleague's code involving **Documenting why each choice was made** produces results that look plausible but are wrong.
