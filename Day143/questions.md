...
Someone writes code for **Chain-of-thought prompting** and hits a bug in production.
...
Read the example for **Extended thinking modes** in `examples/` without running it.
...
For **Tool use as reasoning support** the rule is:

> Validate every model-produced payload against the schema before it reaches your database.
...
You are building a real system and **Where reasoning helps and where it wastes tokens** is on the table.
...
A colleague's code involving **Choosing a reasoning budget** produces results that look plausible but are wrong.
