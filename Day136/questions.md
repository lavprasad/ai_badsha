...
Someone writes code for **Fluency versus factuality** and hits a bug in production.
...
Read the example for **Grounding in sources** in `examples/` without running it.
...
For **Self-consistency sampling** the rule is:
...
You are building a real system and **Abstaining when uncertain** is on the table.
...
A colleague's code involving **Designing for graceful wrongness** produces results that look plausible but are wrong.
