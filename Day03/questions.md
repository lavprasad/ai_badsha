...
Someone writes code for **List comprehensions** and hits a bug in production.
...
Read the example for **Generators and yield** in `examples/` without running it.
...
For **map, filter and lambda** the rule is:

> `Counter` and `defaultdict` remove most of the bookkeeping code people write by hand.
...
You are building a real system and **collections: Counter, defaultdict** is on the table.
...
A colleague's code involving **Choosing the right container** produces results that look plausible but are wrong.
