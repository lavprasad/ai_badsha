...
Someone writes code for **Choosing a feature worth AI** and hits a bug in production.
...
Read the example for **Progressive disclosure of AI output** in `examples/` without running it.
...
For **Explaining what the AI did** the rule is:
...
You are building a real system and **Onboarding users to a fuzzy tool** is on the table.
...
A colleague's code involving **Knowing when to remove the AI** produces results that look plausible but are wrong.
