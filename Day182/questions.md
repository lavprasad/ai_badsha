...
Someone writes code for **The three things to monitor** and hits a bug in production.
...
Read the example for **Prediction drift** in `examples/` without running it.
...
For **Population Stability Index** the rule is:
...
You are building a real system and **Segment-level monitoring** is on the table.
...
A colleague's code involving **An on-call runbook for a model** produces results that look plausible but are wrong.
