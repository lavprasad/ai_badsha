...
Someone writes code for **A matrix as a transformation** and hits a bug in production.
...
Read the example for **Identity and inverse matrices** in `examples/` without running it.
...
For **Determinant and what zero means** the rule is:
...
You are building a real system and **Solving linear systems with solve** is on the table.
...
A colleague's code involving **Batched matrix operations** produces results that look plausible but are wrong.
