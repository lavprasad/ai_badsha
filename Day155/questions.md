...
Someone writes code for **Prompt template for grounded answers** and hits a bug in production.
...
Read the example for **Instructing abstention** in `examples/` without running it.
...
For **Answer verification against sources** the rule is:
...
You are building a real system and **Context ordering effects** is on the table.
...
A colleague's code involving **Measuring groundedness** produces results that look plausible but are wrong.
