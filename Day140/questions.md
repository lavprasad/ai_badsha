...
Someone writes code for **Waveforms and sampling rates** and hits a bug in production.
...
Read the example for **Automatic speech recognition** in `examples/` without running it.
...
For **Diarisation: who spoke when** the rule is:

> Resample everything to the model's expected sample rate before inference.
...
You are building a real system and **Voice cloning and its ethics** is on the table.
...
A colleague's code involving **A meeting transcription pipeline** produces results that look plausible but are wrong.
