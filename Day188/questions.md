...
Someone writes code for **Components of a production AI system** and hits a bug in production.
...
Read the example for **Queues and workers** in `examples/` without running it.
...
For **Fallback and degradation paths** the rule is:
...
You are building a real system and **Vendor lock-in and abstraction layers** is on the table.
...
A colleague's code involving **Drawing your system on one page** produces results that look plausible but are wrong.
