...
Someone writes code for **Why full fine-tuning is expensive** and hits a bug in production.
...
Read the example for **LoRA: the low-rank idea** in `examples/` without running it.
...
For **QLoRA and 4-bit base weights** the rule is:

> Fix the random seed and log every trial, or you cannot reproduce your own best model.
...
You are building a real system and **Merging adapters back** is on the table.
...
A colleague's code involving **Fine-tuning a small model on one GPU** produces results that look plausible but are wrong.
