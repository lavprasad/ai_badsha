...
Someone writes code for **Why zero initialisation fails** and hits a bug in production.
...
Read the example for **Xavier/Glorot initialisation** in `examples/` without running it.
...
For **Batch normalisation** the rule is:
...
You are building a real system and **Layer normalisation** is on the table.
...
A colleague's code involving **Diagnosing activation blow-up** produces results that look plausible but are wrong.
