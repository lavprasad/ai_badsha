...
Someone writes code for **Face detection vs recognition** and hits a bug in production.
...
Read the example for **Landmark detection** in `examples/` without running it.
...
For **Person re-identification** the rule is:

> Biometric data usually cannot be revoked. Treat it as the most sensitive category you handle.
...
You are building a real system and **Privacy and consent** is on the table.
...
A colleague's code involving **Safer alternatives to face recognition** produces results that look plausible but are wrong.
