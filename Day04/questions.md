...
Someone writes code for **Reading and writing text files** and hits a bug in production.
...
Read the example for **CSV files with the csv module** in `examples/` without running it.
...
For **pathlib for portable paths** the rule is:

> Use `Path` division (`root / 'data' / 'x.csv'`) instead of string concatenation with slashes.
...
You are building a real system and **Working with large files line by line** is on the table.
...
A colleague's code involving **A tiny dataset downloader script** produces results that look plausible but are wrong.
