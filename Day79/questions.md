...
Someone writes code for **The computational graph** and hits a bug in production.
...
Read the example for **The chain rule, backwards** in `examples/` without running it.
...
For **Backprop through a two-layer network** the rule is:

> Gradient-check any hand-written backward pass against a numeric estimate before trusting it.
...
You are building a real system and **Gradient checking** is on the table.
...
A colleague's code involving **Implementing a full training loop in NumPy** produces results that look plausible but are wrong.
