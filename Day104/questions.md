...
Someone writes code for **Goal: a fine-tuned classifier you can defend** and hits a bug in production.
...
Read the example for **Stratified splits and a locked test set** in `examples/` without running it.
...
For **Transfer learning with a pretrained backbone** the rule is:

> Use the exact normalisation statistics the pretrained model was trained with.
...
You are building a real system and **Training loop with checkpointing** is on the table.
...
A colleague's code involving **A short model card** produces results that look plausible but are wrong.
