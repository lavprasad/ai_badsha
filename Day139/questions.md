...
Someone writes code for **Tokenisation cost across scripts** and hits a bug in production.
...
Read the example for **Cross-lingual transfer** in `examples/` without running it.
...
For **Code-mixed Hinglish text** the rule is:

> Measure tokens per request in your users' actual language, not in English.
...
You are building a real system and **Low-resource language strategies** is on the table.
...
A colleague's code involving **Building for Indian language users** produces results that look plausible but are wrong.
