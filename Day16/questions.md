...
Someone writes code for **What a vector represents in ML** and hits a bug in production.
...
Read the example for **The dot product and its meaning** in `examples/` without running it.
...
For **Unit vectors and normalisation** the rule is:
...
You are building a real system and **Orthogonality and projection** is on the table.
...
A colleague's code involving **Implementing vector ops in NumPy** produces results that look plausible but are wrong.
