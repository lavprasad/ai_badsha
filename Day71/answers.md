...
**When your data stops fitting in memory** — the classic failure is:
...
The example for **Sparse matrices for text** is `examples/03_sparse_matrices_for_text.py`.
...
Key idea to check yourself against: Profile first. GPU at 30% utilisation means the bottleneck is the data pipeline, not the GPU.
...
**Dask and out-of-core dataframes** in one line: A DataFrame is a table with labelled columns and an index.
...
For **Knowing when to move to a database**, check in this order:
