...
Someone writes code for **Overfitting in high-capacity models** and hits a bug in production.
...
Read the example for **Dropout** in `examples/` without running it.
...
For **Early stopping** the rule is:
...
You are building a real system and **Label smoothing** is on the table.
...
A colleague's code involving **Choosing regularisation by symptom** produces results that look plausible but are wrong.
