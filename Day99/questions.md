...
Someone writes code for **Agent, environment, reward** and hits a bug in production.
...
Read the example for **Policy and value functions** in `examples/` without running it.
...
For **Exploration vs exploitation** the rule is:
...
You are building a real system and **Q-learning** is on the table.
...
A colleague's code involving **Reward hacking** produces results that look plausible but are wrong.
