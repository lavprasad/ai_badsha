...
Someone writes code for **Document ingestion pipeline** and hits a bug in production.
...
Read the example for **PDF text vs scanned PDF** in `examples/` without running it.
...
For **Layout and reading order** the rule is:

> Arithmetic and schema checks on extracted fields catch OCR errors that no confidence score flags.
...
You are building a real system and **Metadata and provenance** is on the table.
...
A colleague's code involving **A document intelligence service** produces results that look plausible but are wrong.
