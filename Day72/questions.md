...
Someone writes code for **Check the data before the model** and hits a bug in production.
...
Read the example for **Overfit a tiny subset deliberately** in `examples/` without running it.
...
For **Inspect the loss curve** the rule is:

> Expected cross-entropy at initialisation is ln(n_classes). A different value means a wiring bug.
...
You are building a real system and **Look at the worst predictions by hand** is on the table.
...
A colleague's code involving **A systematic debugging checklist** produces results that look plausible but are wrong.
