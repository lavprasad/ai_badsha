...
Someone writes code for **Why augmentation works** and hits a bug in production.
...
Read the example for **Colour jitter and photometric transforms** in `examples/` without running it.
...
For **Cutout, mixup and CutMix** the rule is:
...
You are building a real system and **Albumentations pipelines** is on the table.
...
A colleague's code involving **Visualising augmented batches** produces results that look plausible but are wrong.
