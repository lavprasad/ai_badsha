...
Someone writes code for **Splitting to increase purity** and hits a bug in production.
...
Read the example for **Information gain** in `examples/` without running it.
...
For **Depth, leaf size and pruning** the rule is:

> A single unpruned tree is almost always worse than a small forest — but it is readable, which sometimes wins.
...
You are building a real system and **Regression trees** is on the table.
...
A colleague's code involving **Visualising and exporting a tree** produces results that look plausible but are wrong.
