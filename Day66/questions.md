...
Someone writes code for **What makes time series different** and hits a bug in production.
...
Read the example for **Stationarity and differencing** in `examples/` without running it.
...
You are building a real system and **Prophet-style additive models** is on the table.
...
A colleague's code involving **Forecast evaluation and backtesting** produces results that look plausible but are wrong.
