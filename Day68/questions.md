...
Someone writes code for **Prediction vs intervention** and hits a bug in production.
...
Read the example for **Randomised experiments as the gold standard** in `examples/` without running it.
...
For **Difference-in-differences** the rule is:

> Name the confounders before you interpret any observational effect. If you cannot, do not claim causality.
...
You are building a real system and **Directed acyclic graphs** is on the table.
...
A colleague's code involving **Asking causal questions honestly** produces results that look plausible but are wrong.
