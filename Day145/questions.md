...
Someone writes code for **Why small models matter** and hits a bug in production.
...
Read the example for **Task-specific small models** in `examples/` without running it.
...
For **GGUF quantisation levels** the rule is:
...
You are building a real system and **Privacy advantages of local inference** is on the table.
...
A colleague's code involving **Routing between small and large models** produces results that look plausible but are wrong.
