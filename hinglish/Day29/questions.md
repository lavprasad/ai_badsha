...
Koi **Floating point representation** ke liye code likhta hai aur production me bug aa jaata hai.
...
`examples/` me **Overflow and underflow** wali file bina chalaye padho.
...
**Epsilon in denominators** ka rule hai:

> Floats ko tolerance se compare karo (`np.isclose`), `==` se kabhi nahi.
...
Aap ek asli system bana rahe ho aur **Deterministic seeds vs true randomness** vichaar me hai.
...
Aapke colleague ka **Debugging a silently wrong computation** wala code aise results de raha hai jo theek lagte hain par galat hain.
