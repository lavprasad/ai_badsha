...
Koi **Loss as the objective you actually optimise** ke liye code likhta hai aur production me bug aa jaata hai.
...
`examples/` me **Mean absolute error and Huber** wali file bina chalaye padho.
...
**Categorical cross-entropy** ka rule hai:

> `log` se pehle probabilities clip karo — `log(0)` `-inf` hai aur poora batch kharab kar deta hai.
...
Aap ek asli system bana rahe ho aur **Class weights inside the loss** vichaar me hai.
...
Aapke colleague ka **Custom losses for business costs** wala code aise results de raha hai jo theek lagte hain par galat hain.
