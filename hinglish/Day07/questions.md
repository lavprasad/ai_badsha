...
Koi **Element-wise arithmetic** ke liye code likhta hai aur production me bug aa jaata hai.
...
`examples/` me **Reductions: sum, mean, max along an axis** wali file bina chalaye padho.
...
**Transpose and axis swapping** ka rule hai:

> `axis=0` rows ko collapse karta hai (columns ke neeche); `axis=1` columns ko (ek row ke aar-paar).
...
Aap ek asli system bana rahe ho aur **np.where and conditional logic** vichaar me hai.
...
Aapke colleague ka **Common shape errors and how to read them** wala code aise results de raha hai jo theek lagte hain par galat hain.
