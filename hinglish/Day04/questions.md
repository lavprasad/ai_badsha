...
Koi **Reading and writing text files** ke liye code likhta hai aur production me bug aa jaata hai.
...
`examples/` me **CSV files with the csv module** wali file bina chalaye padho.
...
**pathlib for portable paths** ka rule hai:

> Slashes wali string joduai ke bajaye `Path` division (`root / 'data' / 'x.csv'`) use karo.
...
Aap ek asli system bana rahe ho aur **Working with large files line by line** vichaar me hai.
...
Aapke colleague ka **A tiny dataset downloader script** wala code aise results de raha hai jo theek lagte hain par galat hain.
