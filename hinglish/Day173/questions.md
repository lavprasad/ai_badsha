...
Koi **When one agent is not enough** ke liye code likhta hai aur production me bug aa jaata hai.
...
`examples/` me **Parallel fan-out and synthesis** wali file bina chalaye padho.
...
**Shared state and handoffs** ka rule hai:

> Loop hamesha bounded rakho. Bina limit ka agent paisa jalata hai aur fail hone ke naye tareeke dhoondh leta hai.
...
Aap ek asli system bana rahe ho aur **Debugging multi-agent runs** vichaar me hai.
...
Aapke colleague ka **Simplifying back to one agent** wala code aise results de raha hai jo theek lagte hain par galat hain.
