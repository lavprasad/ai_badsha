...
Someone writes code for **MAE and its interpretation** and hits a bug in production.
...
Read the example for **R-squared and adjusted R-squared** in `examples/` without running it.
...
For **Choosing a metric from the cost of error** the rule is:
...
You are building a real system and **Residual analysis** is on the table.
...
A colleague's code involving **Reporting results without overclaiming** produces results that look plausible but are wrong.
