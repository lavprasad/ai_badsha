...
Someone writes code for **Retrieval is the bottleneck, not generation** and hits a bug in production.
...
Read the example for **Query rewriting** in `examples/` without running it.
...
For **Multi-query retrieval** the rule is:
...
You are building a real system and **Reranking with a cross-encoder** is on the table.
...
A colleague's code involving **Building a retrieval eval set** produces results that look plausible but are wrong.
