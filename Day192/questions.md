...
Someone writes code for **Problem framing and metric** and hits a bug in production.
...
Read the example for **EDA and leakage audit** in `examples/` without running it.
...
For **Model selection with honest CV** the rule is:
...
You are building a real system and **Calibration and threshold choice** is on the table.
...
A colleague's code involving **Write-up with results and limitations** produces results that look plausible but are wrong.
