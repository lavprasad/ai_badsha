...
Someone writes code for **The confusion matrix** and hits a bug in production.
...
Read the example for **Precision and recall** in `examples/` without running it.
...
For **Specificity and false positive rate** the rule is:
...
You are building a real system and **Precision-recall curves for imbalance** is on the table.
...
A colleague's code involving **Choosing the metric before the model** produces results that look plausible but are wrong.
