...
Someone writes code for **A prompt is a specification** and hits a bug in production.
...
Read the example for **Zero-shot vs few-shot** in `examples/` without running it.
...
For **Delimiters and structure** the rule is:
...
You are building a real system and **Chain-of-thought when useful** is on the table.
...
A colleague's code involving **Versioning prompts like code** produces results that look plausible but are wrong.
