...
Someone writes code for **Public dataset sources** and hits a bug in production.
...
Read the example for **Web scraping ethics and robots.txt** in `examples/` without running it.
...
For **File formats: CSV, Parquet, JSONL** the rule is:
...
You are building a real system and **Data licensing and terms of use** is on the table.
...
A colleague's code involving **Documenting data provenance** produces results that look plausible but are wrong.
