...
Someone writes code for **Unicode, encodings and normalisation** and hits a bug in production.
...
Read the example for **Case folding trade-offs** in `examples/` without running it.
...
For **Sentence splitting** the rule is:
...
You are building a real system and **Handling code-mixed text** is on the table.
...
A colleague's code involving **A reusable text cleaning function** produces results that look plausible but are wrong.
