...
**Continuous integration basics** — the classic failure is:
...
The example for **Running tests on every push** is `examples/03_running_tests_on_every_push.py`.
...
**Blue-green and canary deploys** in one line: ML code needs the same tests as any code, plus data tests: schema, ranges, null rates, class balance.
...
For **A pipeline that blocks bad models**, check in this order:
