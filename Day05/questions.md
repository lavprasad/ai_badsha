...
Someone writes code for **Classes and instances** and hits a bug in production.
...
Read the example for **Dunder methods: __repr__, __len__** in `examples/` without running it.
...
For **Inheritance vs composition** the rule is:

> In a dataclass, mutable defaults need `field(default_factory=...)`, not a literal.
...
You are building a real system and **Custom exception classes** is on the table.
...
A colleague's code involving **Writing code your future self can read** produces results that look plausible but are wrong.
