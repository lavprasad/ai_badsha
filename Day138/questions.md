...
Someone writes code for **Turns, roles and system prompts** and hits a bug in production.
...
Read the example for **Context window management** in `examples/` without running it.
...
For **Persona and tone control** the rule is:

> A correction must overwrite, not coexist. Two contradictory memories will both get retrieved.
...
You are building a real system and **Escalation to a human** is on the table.
...
A colleague's code involving **Building a grounded support bot** produces results that look plausible but are wrong.
