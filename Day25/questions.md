...
Someone writes code for **Framing a decision as an experiment** and hits a bug in production.
...
Read the example for **Choosing a primary metric** in `examples/` without running it.
...
For **Deciding sample size up front** the rule is:

> Decide the sample size and the metric BEFORE looking at the data.
...
You are building a real system and **Guardrail metrics** is on the table.
...
A colleague's code involving **When an A/B test is the wrong tool** produces results that look plausible but are wrong.
