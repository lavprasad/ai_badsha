...
Someone writes code for **Why a protocol for tools** and hits a bug in production.
...
Read the example for **Resources, tools and prompts** in `examples/` without running it.
...
For **Authentication in integrations** the rule is:

> A tool server's response is untrusted input to your agent. Delimit it and never let it grant permissions.
...
You are building a real system and **Security review of a third-party server** is on the table.
...
A colleague's code involving **Integrations as the real product surface** produces results that look plausible but are wrong.
