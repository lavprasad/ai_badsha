...
**groupby: split, apply, combine** — the classic failure is:
...
The example for **agg with multiple functions** is `examples/03_agg_with_multiple_functions.py`.
...
**melt: wide to long** in one line: A DataFrame is a table with labelled columns and an index.
...
For **Chaining without losing readability**, check in this order:
