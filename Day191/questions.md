...
Someone writes code for **Picking a problem you care about** and hits a bug in production.
...
Read the example for **Data availability check first** in `examples/` without running it.
...
For **Baseline you must beat** the rule is:
...
You are building a real system and **Milestones and a thin first slice** is on the table.
...
A colleague's code involving **Writing the project brief** produces results that look plausible but are wrong.
