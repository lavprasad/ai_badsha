...
Someone writes code for **What a calibrated probability means** and hits a bug in production.
...
Read the example for **Brier score** in `examples/` without running it.
...
For **Isotonic regression** the rule is:
...
You are building a real system and **Calibration under class imbalance** is on the table.
...
A colleague's code involving **Expected value of a decision** produces results that look plausible but are wrong.
