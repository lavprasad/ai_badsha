...
Someone writes code for **Discrete vs continuous distributions** and hits a bug in production.
...
Read the example for **Poisson for counts** in `examples/` without running it.
...
For **The Gaussian and the CLT** the rule is:
...
You are building a real system and **Multivariate Gaussian and covariance** is on the table.
...
A colleague's code involving **How the likelihood picks your loss function** produces results that look plausible but are wrong.
