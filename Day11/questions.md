...
Someone writes code for **Figure and Axes: the object API** and hits a bug in production.
...
Read the example for **Histograms and density** in `examples/` without running it.
...
For **Labels, titles, legends, annotation** the rule is:
...
You are building a real system and **Log scales and when to use them** is on the table.
...
A colleague's code involving **Plot first, model second** produces results that look plausible but are wrong.
