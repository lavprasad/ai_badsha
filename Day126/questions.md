...
Someone writes code for **Query, key, value projections** and hits a bug in production.
...
Read the example for **Causal and padding masks** in `examples/` without running it.
...
For **Attention complexity in sequence length** the rule is:

> The 1/sqrt(d) scale is not cosmetic — without it softmax saturates and gradients die.
...
You are building a real system and **Sliding window and sparse attention** is on the table.
...
A colleague's code involving **Implementing multi-head attention** produces results that look plausible but are wrong.
