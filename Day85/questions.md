...
Someone writes code for **Dataset and __getitem__** and hits a bug in production.
...
Read the example for **num_workers and prefetching** in `examples/` without running it.
...
For **Transforms and augmentation** the rule is:
...
You are building a real system and **Weighted samplers for imbalance** is on the table.
...
A colleague's code involving **Building a Dataset for your own files** produces results that look plausible but are wrong.
