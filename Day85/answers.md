...
**Dataset and __getitem__** — the classic failure is:
...
The example for **num_workers and prefetching** is `examples/03_num_workers_and_prefetching.py`.
...
**Weighted samplers for imbalance** in one line: A Pipeline chains preprocessing and the model into one object that fits and predicts as a unit.
...
For **Building a Dataset for your own files**, check in this order:
