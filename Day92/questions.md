...
Someone writes code for **Sequential data and order** and hits a bug in production.
...
Read the example for **Backpropagation through time** in `examples/` without running it.
...
For **LSTM gates** the rule is:

> RNNs are inherently sequential — they cannot parallelise across time, which is why transformers won.
...
You are building a real system and **Bidirectional RNNs** is on the table.
...
A colleague's code involving **Why transformers replaced them** produces results that look plausible but are wrong.
