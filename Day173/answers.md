...
**When one agent is not enough** — the classic failure is:
...
The example for **Parallel fan-out and synthesis** is `examples/03_parallel_fan_out_and_synthesis.py`.
...
**Debugging multi-agent runs** in one line: An agent is a loop: the model picks a tool, your code runs it, the result goes back into context, repeat until done.
...
For **Simplifying back to one agent**, check in this order:
