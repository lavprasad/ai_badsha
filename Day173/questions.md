...
Someone writes code for **When one agent is not enough** and hits a bug in production.
...
Read the example for **Parallel fan-out and synthesis** in `examples/` without running it.
...
For **Shared state and handoffs** the rule is:
...
You are building a real system and **Debugging multi-agent runs** is on the table.
...
A colleague's code involving **Simplifying back to one agent** produces results that look plausible but are wrong.
